...
export async function fetchPublicPosts(): Promise<PromptPost[]> {
  ...
  return (data ?? []).map((row: any) => {
    ...
    const url = (type: string) => { const item = images.find((x: any) => x.image_type === type); return item ? supabase!.storage.from('post-images').getPublicUrl(item.storage_path).data.publicUrl : '/demo-after.svg' }
